...
<?php
    ...
    foreach ($txts as $txt) {
        $vcount ++;
        $txt_overlay .= ($vcount==1?"[0:v]":"[dest".($vcount-1)."]")." drawtext=fontfile=../fonts/Songti.ttc:text='".($txt->text)."':fontcolor=red:fontsize=48:x=".($txt->left).":y=".($txt->top).":enable='between(t,".($txt->begin).",".($txt->end).")'[dest".$vcount."];";
    }
